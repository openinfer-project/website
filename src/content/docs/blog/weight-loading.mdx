---
title: "OpenInfer Weight Loading: The Complete Data Path from Safetensors to GPU"
description: "Explains how OpenInfer locates weights from the Safetensors index, handles TP/FP8/EP sharding, and uses two-slot pinned staging to overlap CPU preparation with asynchronous H2D—and why a faster local stage did not lead to a faster total time."
---
...
**OpenInfer starts a Qwen3-4B service in just 2.36 seconds.**<br />**Even the roughly 700 GiB GLM-5.2-FP8 takes only 25.73 seconds.**
...
> This article first explains how OpenInfer loads weights and how it optimizes weight loading step by step. It then analyzes a set of independent transfer experiments.

## TL;DR

- **How weights move from Safetensors to the GPU**
- **How TP, FP8, and EP change the loading process**
- **How OpenInfer optimizes the transfer path**
...
OpenInfer's Qwen3 TP uses this path for `o_proj` and `down_proj`. Take `o_proj` as an example:
...
How does OpenInfer optimize the time spent in this stage?
...
These three are comparison paths in the microbenchmark, not OpenInfer Qwen3 runtime options; OpenInfer Qwen3 currently uses double pinned staging.
...
In most tested cases, however, staging is the better one, so OpenInfer currently uses staging.
...
After the optimizations above, with a warm page cache, how long does it take from the user entering the startup command until OpenInfer can provide an HTTP service? The table below shows the results of 10 runs.
